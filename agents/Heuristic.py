...
class Heuristic(Agent):
    ...
    def __init__(self, name='ProIntelligentAgent'):
        '''
    ...
    def mission_outcome(self, mission, proposer, num_betrayals, mission_success):
        ...
        if num_betrayals > 0:
            # Update suspicion based on the likelihood of betrayal
            for player in mission:
                if player != self.player_number:
                    # Calculate the probability that this player is a spy
                    # given that the mission failed
                    prior = self.suspicion[player]
                    likelihood = num_betrayals / len(mission)
                    posterior = prior + (1 - prior) * likelihood
                    self.suspicion[player] = posterior
        else:
            # Decrease suspicion for team members
            for player in mission:
                if player != self.player_number:
                    self.suspicion[player] *= 0.9  # Reduce suspicion by 10%
